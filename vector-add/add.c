...
int vect_add_linear_multistage(const int *a, const int *b, int *c, int len)
{
    int i;
...
int vect_add_parallel_multistage(struct clparams *clp, struct addparams *addp,
        const int *a, const int *b, int *c, int len)
{
    cl_int ret;
    ...
    /* Define os argumentos para o kernel. */
    ret = clSetKernelArg(clp->kernel, 0, sizeof(cl_mem), (void *)&addp->a);
    ret = clSetKernelArg(clp->kernel, 1, sizeof(cl_mem), (void *)&addp->c);
    ret = clSetKernelArg(clp->kernel, 2, sizeof(cl_mem), (void *)&addp->c);
